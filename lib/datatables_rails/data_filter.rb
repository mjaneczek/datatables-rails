module DatatablesRails
  class DataFilter
    ...
    def get_filtered_data(source)
      ...
      source = filter_module.sort_data(source, find_sort_column_name, @request_params.sort_direction)
      source = filter_module.page_data(source, @request_params.page, @request_params.per_page)
    ...
    private 
      ...
      def filter_by_searched_text_if_exists(source)
        if @request_params.search_text.present?
          source = filter_module.search_data(source, @options.search_columns, @request_params.search_text)
        end

        @records_count_after_filter = filter_module.get_total_count(source)
        source
      end
